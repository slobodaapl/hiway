#![doc = "Derive macro for `hiway::HiwayEvent`."]
#![forbid(unsafe_code)]
...
fn expand(input: &DeriveInput) -> Result<TokenStream2> {
    ...
    let payloads = collect_payloads(enum_data)?;
    ...
    let conversions = variants
        ...
        .map(|(tag, (variant, payload))| {
            quote! {
                ...
                impl #impl_generics #hiway::__private::EventTag<#enum_name #ty_generics>
                    for #payload #where_clause
                {
    ...
    let tag_matches = enum_data.variants.iter().enumerate().map(|(tag, variant)| {
...
fn collect_payloads(enum_data: &syn::DataEnum) -> Result<Vec<&syn::Type>> {
